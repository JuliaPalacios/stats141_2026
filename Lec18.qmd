...
<a href="Lec18_annotated.pdf" class="download-btn" download> ⬇ Download Annotated Slides </a>
:::
...
where $\sigma^{2}=Var(\epsilon)$ is the variance of the error terms, which is unknown but can be estimated empirically via the RSE (residual standard error) defined as follows:
...
Once we obtain the standard errors of the estimates, we can construct *confidence intervals*. For the linear regression model, the 95% confidence intervals for $\beta_{0}$ and $\beta_{1}$ are approximated as:
              
$$\left[\hat{\beta}_{0}-t_{.975,n-2}\cdot SE(\hat{\beta}_{0}),\ \hat{\beta}_{0}+t_{.975,n-2}\cdot SE(\hat{\beta}_{0})\right]$$ {#eq-6} $$\left[\hat{\beta}_{1}-t_{.975,n-2}\cdot SE(\hat{\beta}_{1}),\ \hat{\beta}_{1}+t_{.975,n-2}\cdot SE(\hat{\beta}_{1})\right]$$ {#eq-7}
