...
Under $H_0$, the sample mean follows $\overline{X} \sim N(\mu_0,\; \sigma^2/n)$.

Under the true mean $\mu_A = 22$, we have $\overline{X} \sim N(\mu_A,\; \sigma^2/n)$.
...
Any observed $\overline{x} > c$ leads to rejection of $H_0$.
...
R's `power.t.test` computes power (or required $n$) for one-sample and two-sample $t$-tests. The argument `delta` is the true effect size in original units.
...
|:--------------|:------------------|:----------------------|
| Sample size $n$ $\uparrow$ | SE decreases | Power $\uparrow$ |
| True effect size $\delta$ $\uparrow$ | Signal grows | Power $\uparrow$ |
| Significance level $\alpha$ $\uparrow$ | Rejection region widens | Power $\uparrow$ (but more false positives) |
| Population SD $\sigma$ $\uparrow$ | SE increases, SNR drops | Power $\downarrow$ |
