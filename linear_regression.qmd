...
For example we are interested to know whether a cat's body weight ($X$) and heart weight ($Y$) are associated, specifically, whether bigger cats tend to have bigger hearts.
:::

In this example, we refer to $Y=$heart weight as the *response variable*, also known as the *outcome* or *dependent variable*, and we refer to $X=$body weight as the *input variable*, also known as a *predictor*, *independent variable*, *feature*, or simply a *variable*. The goal of statistical learning is to determine whether a relationship exists between these variables, and to approximate a function that relates the input to the response.
...
::: {.callout-note icon="false"}
## Example: Maternal predictors of birth weight
...
**The birthwt dataset** in the `MASS` package contains 189 observations collected at Baystate Medical Center, Springfield, Massachusetts in 1986. The potential predictor variables are listed in the table below.
