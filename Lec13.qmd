...
![random cat](/Figures/cat_randomization.png){fig-align="center" width="40%"}
...
Back to the coin toss: we didn’t reject “the coin is fair” at 5% even though it was actually biased (probHead = 0.6). If we try many different test statistics (runs of heads, first 50 flips, etc.), eventually one will give a small p-value just by chance. **This is p-value hacking**. A related practice is **HARKing:** changing hypotheses after seeing the data until something looks significant.

These violate proper hypothesis testing, but they’re tempting with messy data (many preprocessing and modeling choices). The key is transparency: report what you tried, how you did it, and share code—only then is a p-value interpretable.

Keep in mind that our test is never attempting to prove the null hypothesis, we are simply saying whether there is evidence foor it to be false.

### Multiple Testing

In the coin example, we would expect $100\times .05$ false positives, why? Now, what is the chance that we get at least one false positive? $$1-\text{P(none is false positive)}=1-(.95)^{100}=0.99 $$ A [`classic example`](https://xkcd.com/882/) is the "association" of green jelly beans and acne .
...
| Truth / Decision | Reject $H_0$ | Fail to reject $H_0$ |
|:--------------------|:------------------------|:-------------------------|
| $H_0$ is true | $V$ False positives | $U$ (No error) |
| $H_0$ is false ($H_1$ true) | $S$ (correct) | $T$ False negative |
| Total | $R$ | $m-R$ |
